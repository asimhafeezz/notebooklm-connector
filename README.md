...
- **Supported browsers** — Chrome, Brave, Edge, Arc, Opera, Vivaldi, Firefox, LibreWolf, Zen (easiest — one Keychain tap), plus Safari (needs macOS **Full Disk Access** for the app running the connector). Just say which one: *“connect using Brave.”*
  On an unsupported browser (e.g. Comet)? Sign in to [notebooklm.google.com](https://notebooklm.google.com) once in any supported browser — **Chrome, Brave, or Firefox are the smoothest** — then *“connect using Brave.”* Or use the interactive sign-in window (`uv sync --extra interactive-login && uv run playwright install chromium`, then *“log in interactively”*), which works no matter your daily browser.
